...
docker build --rm --network host --build-arg http_proxy=$http_proxy --build-arg https_proxy=$https_proxy -f ${WRKDIR}/Dockerfile.local -t safescale:issues ${WRKDIR}
[ $? -ne 0 ] && echo "Docker build failed !!" && rm -f ./marker && return 1

echo "Docker build OK"

docker create -ti --name dummy safescale:issues bash
