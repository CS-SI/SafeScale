...
// pattern.go modified from the origal docopt-go to return arguments parsed even on error
package docopt
...
func (p *pattern) match(left *patternList, collected *patternList) (bool, *patternList, *patternList) {
	...
	if p.t&patternOptionAL != 0 || p.t&patternOptionSSHORTCUT != 0 {
		for _, p := range p.children {
	...
	if p.t&patternEither != 0 {
		...
		// We want the longer "failed outcome" found
		if len(failures) > 0 {
			maxLen := 0
			maxIndex := 0
			for i, v := range failures {
				if len(*v.collected) > maxLen {
					maxLen = len(*v.collected)
					maxIndex = i
				}
			}
			if maxLen > 0 {
				collected = failures[maxIndex].collected
			}
		}
		return false, left, collected
	}
