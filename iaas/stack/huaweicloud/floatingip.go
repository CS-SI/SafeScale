...
func (s *Stack) ListFloatingIPs() pagination.Pager {
	url := s.osclt.Network.Endpoint + "v1/" + s.AuthOpts.ProjectID + "/publicips"
	return pagination.NewPager(s.osclt.Network, url, func(r pagination.PageResult) pagination.Page {
...
func (s *Stack) GetFloatingIP(id string) (*FloatingIP, error) {
	r := getResult{}
	url := s.osclt.Network.Endpoint + "v1/" + s.AuthOpts.ProjectID + "/publicips/" + id
...
func (s *Stack) FindFloatingIPByIP(ipAddress string) (*FloatingIP, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to browser Floating IPs: %s", stack_openstack.ProviderErrorToString(err))
	}
	if found {
		return &fip, nil
	}
...
func (s *Stack) CreateFloatingIP() (*FloatingIP, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to build request to create FloatingIP: %s", stack_openstack.ProviderErrorToString(err))
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to request Floating IP creation: %s", stack_openstack.ProviderErrorToString(err))
	}
	fip, err := r.Extract()
	if err != nil {
...
func (s *Stack) DeleteFloatingIP(id string) error {
	...
	_, r.Err = s.osclt.Driver.Request("DELETE", url, &opts)
	err := r.ExtractErr()
	return err
}

// AssociateFloatingIP to host
func (s *Stack) AssociateFloatingIP(host *model.Host, id string) error {
	...
	_, r.Err = s.osclt.Compute.Post(s.osclt.Compute.ServiceURL("servers", host.ID, "action"), b, nil, nil)
	err = r.ExtractErr()
	if err != nil {
		return fmt.Errorf("failed to associate Floating IP id '%s' to host '%s': %s", id, host.Name, stack_openstack.ProviderErrorToString(err))
	}
	return nil
}

// DissociateFloatingIP from host
func (s *Stack) DissociateFloatingIP(host *model.Host, id string) error {
	fip, err := s.GetFloatingIP(id)
	if err != nil {
		return fmt.Errorf("failed to associate Floating IP id '%s' to host '%s': %s", id, host.Name, stack_openstack.ProviderErrorToString(err))
	}
