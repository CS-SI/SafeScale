...
func ClusterK8S(t *testing.T, provider Providers.Enum) {
	...
	outCreate, err := GetOutput("deploy -v -d cluster create + --cidr 168.192.200.0/24 --disable remotedesktop " + names.Clusters[0])
	require.Nil(t, err)
...
func ClusterSwarm(t *testing.T, provider Providers.Enum) {
	...
	//need --param Password=
	//out, err = GetOutput("deploy -v -d cluster add-feature " + names.Clusters[0] + " remotedesktop")
	//require.Nil(t, err)
