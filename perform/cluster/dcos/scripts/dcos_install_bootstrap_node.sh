...
[ ! -f dcos_generate_config.sh ] && {
    curl -L -q -O https://downloads.dcos.io/dcos/stable/{{.DCOSVersion}}/dcos_generate_config.sh || {
        retcode=$?
        echo "Failed to download dcos_generate_config.sh script"
        exit $?
    }
}
...
docker run -d -p 80:80 -v $PWD/genconf/serve:/usr/share/nginx/html:ro --name nginx nginx >/dev/null || {
    retcode=$?
...
[ ! -f $PWD/genconf/serve/dcos.bin ] && {
    curl -L -q -o $PWD/genconf/serve/dcos.bin https://downloads.dcos.io/binaries/cli/linux/x86-64/dcos-1.8/dcos || {
        retcode=$?
        echo "Failed to download dcos binary"
        exit $retcode
    }
}
