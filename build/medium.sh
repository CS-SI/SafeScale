...
for fla in "${flavor[@]}"; do
	RUN=0

	for i in $(seq $ROUNDS); do
		./safescale cluster delete clu-$TENANT-$fla-r$i -y
		./safescale cluster create -C Medium -F $fla --os "ubuntu 18.04" --sizing "cpu=2,ram>=2,disk>=10" --cidr 10.4.$i.0/24 clu-$TENANT-$fla-r$i
		RUN=$?
		if [[ $RUN -ne 0 ]]; then
			CODE=$((CODE + 1))
		fi
	...
	for i in $(seq $ROUNDS); do
		./safescale cluster delete clu-$TENANT-$fla-r$i -y
		./safescale cluster create -C Medium -F $fla --os "centos 7" --sizing "cpu=2,ram>=2,disk>=10" --cidr 10.7.$i.0/24 clu-$TENANT-$fla-r$i
		RUN=$?
		if [[ $RUN -ne 0 ]]; then
			CODE=$((CODE + 1))
		fi
