...
mintest: begin
	...
	@if [ -s ./test_results.log ] && grep -e '--- FAIL' ./test_results.log 2>&1 > /dev/null; then printf "%b" "$(ERROR_COLOR)$(ERROR_STRING) minimal tests FAILED ! Take a look at ./test_results.log $(NO_COLOR)\n";else printf "%b" "$(OK_COLOR)$(OK_STRING) CONGRATS. TESTS PASSED ! $(NO_COLOR)\n";fi;
	@if [ -s ./test_results.log ] && grep -e '--- FAIL' ./test_results.log; then exit 1;else $(RM) ./test_results.log;fi;
...
test: begin coverdeps # Run unit tests
	...
	@if [ -s ./test_results.log ] && grep -e '--- FAIL' ./test_results.log; then printf "%b" "$(ERROR_COLOR)$(ERROR_STRING) tests FAILED ! Take a look at ./test_results.log $(NO_COLOR)\n";else printf "%b" "$(OK_COLOR)$(OK_STRING) CONGRATS. TESTS PASSED ! $(NO_COLOR)\n";fi;
