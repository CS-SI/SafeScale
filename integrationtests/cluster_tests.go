...
func ClusterK8S(t *testing.T, provider providers.Enum) {
	...
	_, err := GetOutput("safescale -v -d cluster create + --cidr 192.168.200.0/24 --disable remotedesktop " + names.Clusters[0])
	require.Nil(t, err)

	command := "sudo -u cladm -i kubectl run hello-world-za --image=gcr.io/google-samples/node-hello:1.0  --port=8080"
	_, err = GetOutput("safescale ssh run " + names.Clusters[0] + "-master-1 -c \"" + command + "\"")
	require.Nil(t, err)
	...
	_, err = GetOutput("safescale cluster inspect " + names.Clusters[0])
	require.Nil(t, err)
...
func ClusterSwarm(t *testing.T, provider providers.Enum) {
	...
	_, err := GetOutput("safescale -v -d cluster create + --cidr 192.168.201.0/24 --disable remotedesktop --flavor SWARM " + names.Clusters[0])
	require.Nil(t, err)
	...
	_, err = GetOutput("safescale host check-feature gw-net-" + names.Clusters[0] + " kong")
	require.Nil(t, err)

	//need --param Password=
	//_, err = GetOutput("safescale -v -d cluster add-feature " + names.Clusters[0] + " remotedesktop --skip-proxy")
	//require.Nil(t, err)

	//_, err = GetOutput("safescale -v -d cluster check-feature " + names.Clusters[0] + " remotedesktop")
	//require.Nil(t, err)

	//_, err = GetOutput("safescale -v -d cluster delete-feature " + names.Clusters[0] + " remotedesktop")
	//require.Nil(t, err)

	_, err = GetOutput("safescale cluster inspect " + names.Clusters[0])
	require.Nil(t, err)

	_, err = GetOutput("safescale cluster delete --yes " + names.Clusters[0])
	require.Nil(t, err)
}
