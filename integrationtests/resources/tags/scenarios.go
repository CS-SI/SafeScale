//go:build (integration && tagtests) || allintegration
// +build integration,tagtests allintegration
...
func TagError(t *testing.T) {
	names := helpers.GetNames("TagError", 0, 1, 1, 1, 1, 0)
	names.TearDown()
	defer names.TearDown()
	...
	out, err = helpers.GetOutput("safescale host tag add " + names.Hosts[0] + " " + names.Tags[0])
	require.Nil(t, err)
	_ = out
...
func init() {
	helpers.InSection("tags").
		AddScenario(TagError)
}
