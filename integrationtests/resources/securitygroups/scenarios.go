...
func EnableDisableListHostSecurityGroup(t *testing.T) {
	names := helpers.GetNames("sgtest", 0, 0, 0, 0, 1, 0, 0, 0)
	names.TearDown()
	defer names.TearDown()
	...
	// after that it should be open
	out, err = helpers.GetOutput(fmt.Sprintf("safescale network security group rule add --direction ingress --port-from 7777 --port-to 7777 --cidr \"0.0.0.0/0\" --cidr \"%s/32\" %s %s", pubip, names.Networks[0], sgName))
	fmt.Println(out)
	require.Nil(t, err)
	...
	out, err = helpers.GetOutput(fmt.Sprintf("safescale host security group bind %s %s", tn, sgName))
	fmt.Println(out)
	require.Nil(t, err)
	...
	bonds, err := helpers.RunJq(out, ".result.hosts[0].name")
	require.Nil(t, err)
	require.Contains(t, bonds, tn)

	out, err = helpers.GetOutput(fmt.Sprintf("safescale host security group ls %s", tn))
	fmt.Println(out)
	require.Nil(t, err)

	hsg := helpers.HostSgLs{}
	_ = json.Unmarshal([]byte(out), &hsg)

	match := false

	for _, k := range hsg.Result {
		if k.Name == sgName {
			match = true
		}
	}
	...
	out, err = helpers.GetOutput(fmt.Sprintf("safescale host security group unbind %s %s", tn, sgName))
	fmt.Println(out)
	require.Nil(t, err)
