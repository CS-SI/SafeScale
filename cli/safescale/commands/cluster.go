...
func extractClusterArgument(c *cli.Context) error {
	if !c.Command.HasName("list") || strings.HasSuffix(c.App.Name, " node") || strings.HasSuffix(
		...
		if err != nil {
			...
			if c.Command.HasName("create") {
				return clitools.ExitOnErrorWithMessage(
					exitcode.Duplicate, fmt.Sprintf("Cluster '%s' already exists.\n", clusterName),
				)
			}
		}
	}
...
func convertToMap(c api.Cluster) (map[string]interface{}, error) {
	...
	err := properties.LockForRead(property.CompositeV1).ThenUse(
	...
	err = properties.LockForRead(property.NodesV1).ThenUse(
		func(clonable data.Clonable) error {
	...
	err = properties.LockForRead(property.FeaturesV1).ThenUse(
		func(clonable data.Clonable) error {
	...
	err = properties.LockForRead(property.StateV1).ThenUse(
		func(clonable data.Clonable) error {
...
var clusterStateCommand = cli.Command{
	...
	Action: func(c *cli.Context) error {
		...
		return clitools.SuccessResponse(
				...
				if idx+1 < len(args) {
					...
					if localFile != "" {
						...
						rfi := RemoteFileItem{
							Local: localFile,
							Remote: fmt.Sprintf(
								"%s/kubectl_%d.%s.%d.tmp", utils.TempFolder, idx+1, clientID, time.Now().UnixNano(),
							),
							RemoteOwner:  "cladm",
...
var clusterRunCommand = cli.Command{
	...
	Action: func(c *cli.Context) error {
		...
		return clitools.FailureResponse(
			clitools.ExitOnErrorWithMessage(
				exitcode.NotImplemented, "clusterRunCmd not yet implemented",
			),
		)
	},
}
...
func executeCommand(command string, files *RemoteFilesHandler, outs outputs.Enum) error {
	...
	retcode, stdout, stderr, err := safescalessh.Run(
		master, command, outs, temporal.GetConnectionTimeout(), temporal.GetExecutionTimeout(),
	)
	if err != nil {
...
var clusterNodeDeleteCommand = cli.Command{
	...
	Action: func(c *cli.Context) error {
		...
		if !yes && !utils.UserConfirmed(
			fmt.Sprintf(
				"Are you sure you want to delete the node '%s' of the cluster '%s'", hostName, clusterName,
			),
		) {
			return clitools.SuccessResponse("Aborted")
		}
