...
func (client *Client) listAllVolumes() ([]api.Volume, error) {
	...
	if pageErr != nil || len(vs) == 0{
		if pageErr != nil {
			...
			return nil, errors.Wrap(pageErr, fmt.Sprintf("Error listing volume types: %s", openstack.ProviderErrorToString(pageErr)))
		} else {
...
func (client *Client) DeleteVolume(id string) error {
	...
	err = volumes.Delete(client.Volume, id, nil).ExtractErr()
	if err != nil {
		log.Debugf("Error deleting volume: actual delete call: %+v", err)
		return errors.Wrap(err, fmt.Sprintf("Error deleting volume: %s", openstack.ProviderErrorToString(err)))
	}
...
func (client *Client) CreateVolumeAttachment(request api.VolumeAttachmentRequest) (*api.VolumeAttachment, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, fmt.Sprintf("Error creating volume attachment between server %s and volume %s: %s", request.ServerID, request.VolumeID, openstack.ProviderErrorToString(err)))
	}
	...
	if err != nil {
		...
		if detach_err != nil {
			...
			return nil, errors.Wrap(detach_err, fmt.Sprintf("Error deleting volume attachment %s: %s", va.ID, openstack.ProviderErrorToString(detach_err)))
		}
...
func (client *Client) CreateContainer(name string) error {
	...
	if err != nil {
		...
		return errors.Wrap(err, fmt.Sprintf("Error creating container %s: %s", name, openstack.ProviderErrorToString(err)))
	}
...
func (client *Client) DeleteContainer(name string) error {
	...
	if err != nil {
		...
		return errors.Wrap(err, fmt.Sprintf("Error deleting container %s: %s", name, openstack.ProviderErrorToString(err)))
	}
	return err
}
...
func (client *Client) UpdateContainer(name string, meta map[string]string) error {
	...
	if err != nil {
		...
		return errors.Wrap(err, fmt.Sprintf("Error updating container %s: %s", name, openstack.ProviderErrorToString(err)))
	}
	return nil
}
...
func (client *Client) GetContainerMetadata(name string) (map[string]string, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, fmt.Sprintf("Error getting container %s: %s", name, openstack.ProviderErrorToString(err)))
	}
	return meta, nil
...
func (client *Client) ListContainers() ([]string, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, fmt.Sprintf("Error listing containers: %s", openstack.ProviderErrorToString(err)))
	}
...
func (client *Client) PutObject(container string, obj api.Object) error {
	...
	if err != nil {
		...
		return errors.Wrap(err, fmt.Sprintf("Error creating object %s in container %s : %s", obj.Name, container, openstack.ProviderErrorToString(err)))
	}
	return nil
}
...
func (client *Client) GetObject(container string, name string, ranges []api.Range) (*api.Object, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, fmt.Sprintf("Error getting object %s from %s : %s", name, container, openstack.ProviderErrorToString(err)))
	}
...
func (client *Client) GetObjectMetadata(container string, name string) (*api.Object, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, fmt.Sprintf("Error getting object content: %s", openstack.ProviderErrorToString(err)))
	}
	header, err := res.Extract()
	if err != nil {
...
func (client *Client) ListObjects(container string, filter api.ObjectFilter) ([]string, error) {
	...
	if (err != nil) || (len(objectList) == 0) {
		if err != nil {
			...
			return nil, errors.Wrap(err, fmt.Sprintf("Error listing objects of container '%s': %s", container, openstack.ProviderErrorToString(err)))
		}
...
func (client *Client) CopyObject(containerSrc, objectSrc, objectDst string) error {
	...
	if err != nil {
		...
		return errors.Wrap(err, fmt.Sprintf("Error copying object %s into %s from container %s : %s", objectSrc, objectDst, containerSrc, openstack.ProviderErrorToString(err)))
	}
	return nil
}
...
func (client *Client) DeleteObject(container, object string) error {
	...
	if err != nil {
		...
		return errors.Wrap(err, fmt.Sprintf("Error deleting object '%s' of container %s: %s", object, container, openstack.ProviderErrorToString(err)))
	}
	return nil
}
