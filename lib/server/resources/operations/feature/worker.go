...
func newWorker(f resources.Feature, t resources.Targetable, m installmethod.Enum, a installaction.Enum, cb alterCommandCB) (*worker, error) {
	...
	if !f.GetSpecs().IsSet(w.rootKey) {
		msg := `syntax error in feature '%s' specification file (%s):
				no key '%s' found`
		return nil, fmt.Errorf(msg, f.Name(), f.GetDisplayFilename(), w.rootKey)
	}
...
func (w *worker) identifyAllMasters() ([]resources.Host, error) {
	...
	if w.allMasters == nil || len(w.allMasters) == 0 {
		...
		for _, i := range masters {
			host, err := LoadHost(w.feature.task, w.cluster.Service(), i)
			if err != nil {
				return nil, err
			}
...
func (w *worker) identifyAvailableGateway() (resources.Host, error) {
	...
	if w.cluster == nil {
		...
		if err == nil {
			w.availableGateway, err = LoadHost(w.feature.task, w.cluster.Service(), netCfg.GatewayID)
		}
		if err != nil {
			return nil, err
		}
...
func (w *worker) identifyAllGateways() ([]resources.Host, error) {
	...
	if w.cluster != nil {
		...
		network, err = LoadNetwork(w.feature.task, w.cluster.Service(), netCfg.NetworkID)
		if err != nil {
			return nil, err
		}
	} else {
...
func (w *worker) Proceed(v data.Map, s resources.FeatureSettings) (outcomes resources.Results, err error) {
	...
	for _, k := range order {
		...
		if !ok {
			...
			return outcomes, fmt.Errorf(msg, w.feature.Name(), w.feature.GetDisplayFilename(), stepKey)
		}
		params := data.Map{
			"stepName":  k,
			"stepKey":   stepKey,
...
func (w *worker) taskLaunchStep(task concurrency.Task, params concurrency.TaskParameters) (_ concurrency.TaskResult, err error) {
	...
	if w.target.GetTargetType() == featuretargettype.NODE {
		...
		if ok {
			...
			return nil, fmt.Errorf(msg, w.feature.Name(), w.feature.GetDisplayFilename(), stepKey, yamlTargetsKeyword)
		}
	...
	if ok {
		...
		msg := `syntax error in feature '%s' specification file (%s): no key '%s.%s' found`
		return nil, fmt.Errorf(msg, w.feature.Name(), w.feature.GetDisplayFilename(), stepKey, yamlRunKeyword)
	}
