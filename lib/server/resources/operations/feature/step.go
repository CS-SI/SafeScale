...
func (is *step) taskRunOnHost(task concurrency.Task, params concurrency.TaskParameters) (result concurrency.TaskResult, err error) {
	...
	if is.OptionsFileContent != "" {
		// err := operations.UploadStringToRemoteFile(is.OptionsFileContent, host, utils.TempFolder+"/options.json", "cladm:safescale", "ug+rw-x,o-rwx")
		rfcItem := resources.FileRemoteItem{
	...
	// err = operations.UploadStringToRemoteFile(command, host, filename, "", "")
	rfcItem := resources.FileRemoteItem{
		Remote: filename,
	}
