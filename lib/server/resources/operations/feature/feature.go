...
func List(task concurrency.Task, suitableFor string) ([]interface{}, error) {
	...
	for _, feat := range features {
		...
		case "cluster":
			...
			if feat.GetSpecs().IsSet(yamlKey) {
				values := strings.Split(strings.ToLower(feat.GetSpecs().GetString(yamlKey)), ",")
...
// GetFilename returns the name of the feature
func (f *feature) GetFilename() string {
	return f.displayName
}

// GetDisplayFilename returns the full file name, with [embedded] added at the end if the
// feature is embedded.
...
func (f *feature) GetSpecs() *viper.Viper {
	roSpecs := *f.specs
	return &roSpecs
}
...
func (f *feature) Applyable(t resources.Targetable) bool {
	methods := t.GetInstallMethods(f.task)
...
func (f *feature) Check(t resources.Targetable, v data.Map, s resources.FeatureSettings) (_ resources.Results, err error) {
	...
	tracer := concurrency.NewTracer(f.task, fmt.Sprintf("(): '%s' on %s '%s'", f.Name(), t.GetTargetType().String(), t.Name()), true).WithStopwatch().GoingIn()
	defer tracer.OnExitTrace()()
	defer scerr.OnExitLogError(tracer.TraceMessage(""), &err)()
...
func (f *feature) Add(t resources.Targetable, v data.Map, s resources.FeatureSettings) (_ resources.Results, err error) {
	...
	defer temporal.NewStopwatch().OnExitLogInfo(
		fmt.Sprintf("Starting addition of feature '%s' on %s '%s'...", f.Name(), t.GetTargetType().String(), t.Name()),
		fmt.Sprintf("Ending addition of feature '%s' on %s '%s'", f.Name(), t.GetTargetType().String(), t.Name()),
	)()
...
func (f *feature) Remove(t resources.Targetable, v data.Map, s resources.FeatureSettings) (_ resources.Results, err error) {
	...
	defer temporal.NewStopwatch().OnExitLogInfo(
		fmt.Sprintf("Starting removal of feature '%s' from %s '%s'", f.Name(), t.GetTargetType().String(), t.Name()),
		fmt.Sprintf("Ending removal of feature '%s' from %s '%s'", f.Name(), t.GetTargetType().String(), t.Name()),
	)()
