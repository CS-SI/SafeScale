...
func NewKongController(svc iaas.Service, network resources.Network, addressPrimaryGateway bool) (*KongController, error) {
	...
	addressedGateway, err := network.GetGateway(voidtask, addressPrimaryGateway)
	if err != nil {
		return nil, err
	}
	...
	if anon, ok := kongProxyCheckedCache.Get(network.SafeGetName()); ok {
		...
		setErr := kongProxyCheckedCache.SetBy(network.SafeGetName(), func() (interface{}, error) {
			...
			if err != nil {
				return false, fmt.Errorf("failed to check if feature 'edgeproxy4network' is installed on gateway '%s': %s", err.Error(), addressedGateway.SafeGetName())
			}
			if !results.Successful() {
				return false, fmt.Errorf("feature 'edgeproxy4network' isn't installed on gateway '%s'", addressedGateway.SafeGetName())
			}
	...
	if !present {
		return nil, fmt.Errorf("'edgeproxy4network' feature isn't installed on gateway '%s'", addressedGateway.SafeGetName())
	}
	...
	if ctrl.gatewayPrivateIP, err = addressedGateway.GetPrivateIP(voidtask); err != nil {
		return nil, err
	}
	if ctrl.gatewayPublicIP, err = addressedGateway.GetPublicIP(voidtask); err != nil {
		return nil, err
	}
...
func (k *KongController) get(name, url string) (map[string]interface{}, string, error) {
	...
	retcode, stdout, _, err := safescale.New().SSH.Run(task, k.gateway.SafeGetName(), cmd, outputs.COLLECT, temporal.GetConnectionTimeout(), temporal.GetExecutionTimeout())
	if err != nil {
		return nil, "", err
	}
	if retcode != 0 {
...
func (k *KongController) post(name, url, data string, v *data.Map, propagate bool) (map[string]interface{}, string, error) {
	...
	retcode, stdout, stderr, err := safescale.New().SSH.Run(task, k.gateway.SafeGetName(), cmd, outputs.COLLECT, temporal.GetConnectionTimeout(), temporal.GetExecutionTimeout())
	if err != nil {
		return nil, "", err
	}
	if retcode != 0 {
