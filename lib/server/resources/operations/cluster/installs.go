...
func (c *cluster) GetTargetType() featuretargettype.Enum {
...
func (c *cluster) GetInstallMethods(task concurrency.Task) map[uint8]installmethod.Enum {
	if c == nil {
...
func (c *cluster) GetInstalledFeatures(task concurrency.Task) []string {
	var list []string
	return list
}

// FIXME: include the cluster part of setImplicitParameters() from operations.feature
...
func (c *cluster) ComplementFeatureParameters(task concurrency.Task, v data.Map) error {
	...
	if controlPlaneV1.VirtualIP != nil && controlPlaneV1.VirtualIP.PrivateIP != "" {
		...
		v["ClusterControlPlaneEndpointIP"], err = master.PrivateIP(task)
		if err != nil {
			return err
		}
...
func (c *cluster) ExecuteScript(
	...
	// err = operations.UploadStringToRemoteFile(script, host, path, "", "")
...
func (c *cluster) installNodeRequirements(task concurrency.Task, nodeType clusternodetype.Enum, host resources.Host, hostLabel string) (err error) {
	...
	cfg, err := c.service.Config(urationOptions()
	if err == nil {
