...
func TestHost_AddFeature(t *testing.T) {
	...
	err := NewServiceTest(t, func(svc *ServiceTest) {
		...
		// Wrong ctx
		_, xerr = ohost.AddFeature(nil, "ansible", data.Map{}, resources.FeatureSettings{}) // nolint
		require.Contains(t, xerr.Error(), "invalid parameter: ctx")

		// Wrong name
		_, xerr = ohost.AddFeature(ctx, "", data.Map{}, resources.FeatureSettings{})
...
func TestHost_CheckFeature(t *testing.T) {
	...
	err := NewServiceTest(t, func(svc *ServiceTest) {
		...
		// Wrong ctx
		_, xerr = ohost.CheckFeature(nil, "ansible", data.Map{}, resources.FeatureSettings{}) // nolint
		require.Contains(t, xerr.Error(), "invalid parameter: ctx")

		// Wrong name
		_, xerr = ohost.CheckFeature(ctx, "", data.Map{}, resources.FeatureSettings{})
