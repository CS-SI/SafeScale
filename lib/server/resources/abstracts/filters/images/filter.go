...
// Predicate ...
type Predicate func(img abstracts.Image) bool
...
func (f *Filter) Not() *Filter {
	oldFilter := f.filter
	f.filter = func(in abstracts.Image) bool {
		return !oldFilter(in)
	}
	return f
}
...
func Not(f Predicate) Predicate {
	return func(in abstracts.Image) bool {
		return !f(in)
	}
}
...
// FilterImages ...
func FilterImages(images []abstracts.Image, f *Filter) []abstracts.Image {
	res := make([]abstracts.Image, 0)
