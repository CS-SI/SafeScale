...
// Predicate ...
type Predicate func(abstracts.HostTemplate) bool
...
func (f *Filter) Not() *Filter {
	oldFilter := f.filter
	f.filter = func(in abstracts.HostTemplate) bool {
		return !oldFilter(in)
	}
	return f
}
...
func Not(f Predicate) Predicate {
	return func(in abstracts.HostTemplate) bool {
		return !f(in)
	}
}
...
// FilterTemplates ...
func FilterTemplates(templates []abstracts.HostTemplate, f *Filter) []abstracts.HostTemplate {
	res := make([]abstracts.HostTemplate, 0)
