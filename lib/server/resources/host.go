...
type Host interface {
	...
	Create(task concurrency.Task, hostReq abstract.HostRequest, hostDef abstract.HostSizingRequirements) (*userdata.Content, error)           // creates a new host and its metadata
	...
	GetVolumes(task concurrency.Task) (*propertiesv1.HostVolumes, error)                                                                      // returns the volumes attached to the host
	...
	WaitSSHReady(task concurrency.Task, timeout time.Duration) (status string, err error)                                                     // Wait for remote SSH to respond
}
