...
func (c *Controller) DeleteMetadata(task concurrency.Task) error {
	if task == nil {
		task = concurrency.RootTask()
	}
	c.Lock(task)
...
func (c *Controller) DeleteSpecificNode(task concurrency.Task, hostID string, selectedMaster string) error {
	if c == nil {
