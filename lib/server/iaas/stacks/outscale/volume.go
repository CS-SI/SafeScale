...
func (s *Stack) WaitForVolumeState(volumeID string, state volumestate.Enum) (xerr fail.Error) {
	...
	tracer := concurrency.NewTracer(nil, debug.ShouldTrace("stacks.outscale"), "(%s)", volumeID).WithStopwatch().Entering()
	defer tracer.OnExitTrace()
	defer fail.OnExitLogError(tracer.TraceMessage(), &xerr)
...
func (s *Stack) GetVolumeAttachment(serverID, volumeID string) (_ *abstract.VolumeAttachment, xerr fail.Error) {
	...
	tracer := concurrency.NewTracer(nil, debug.ShouldTrace("stacks.outscale"), "(%s, %s)", serverID, volumeID).WithStopwatch().Entering()
	defer tracer.OnExitTrace()
	defer fail.OnExitLogError(tracer.TraceMessage(), &xerr)
...
func (s *Stack) ListVolumeAttachments(serverID string) (_ []abstract.VolumeAttachment, xerr fail.Error) {
	...
	tracer := concurrency.NewTracer(nil, debug.ShouldTrace("stacks.outscale"), "(%s)", serverID).WithStopwatch().Entering()
	defer tracer.OnExitTrace()
	defer fail.OnExitLogError(tracer.TraceMessage(), &xerr)
