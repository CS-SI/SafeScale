...
func (s *Stack) GetNetwork(id string) (*resources.Network, error) {
    ...
    if network != nil && network.ID != "" {
        ...
        if len(sns) != 1 {
            return nil, scerr.Errorf(fmt.Sprintf("bad configuration, each network should have exactly one subnet"), nil)
        }
        sn := sns[0]
...
func (s *Stack) DeleteNetwork(id string) error {
    ...
    if err != nil {
        ...
        return scerr.Errorf(fmt.Sprintf(msg), err)
    }
    for _, sn := range sns {
...
// 	if err != nil {
// 		return "", "", scerr.Errorf(fmt.Sprintf("invalid cidr '%s'", cidr)
// 	}
...
func (s *Stack) listSubnets(netID string) ([]Subnet, error) {
    ...
    paginationErr := pager.EachPage(
        func(page pagination.Page) (bool, error) {
            ...
            if err != nil {
                return false, scerr.Errorf(fmt.Sprintf("error listing subnets: %s", ProviderErrorToString(err)), err)
            }
...
func (s *Stack) CreateVIP(networkID string, name string) (*resources.VirtualIP, error) {
    ...
    if err != nil {
        return nil, scerr.Wrap(err, "error crating VIP")
    }
