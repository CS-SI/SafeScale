...
func (s *Stack) GetNetwork(id string) (_ *resources.Network, err error) {
	...
	if network != nil && network.ID != "" {
		...
		if len(sns) != 1 {
			return nil, fail.Errorf(fmt.Sprintf("bad configuration, each network should have exactly one subnet"), nil)
		}
		sn := sns[0]
...
func (s *Stack) addSubnetToRouter(routerID string, subnetID string) error {
	_, err := routers.AddInterface(
		s.NetworkClient, routerID, routers.AddInterfaceOpts{
			SubnetID: subnetID,
		},
	).Extract()
	if err != nil {
...
func (s *Stack) removeSubnetFromRouter(routerID string, subnetID string) error {
	r := routers.RemoveInterface(
		s.NetworkClient, routerID, routers.RemoveInterfaceOpts{
			SubnetID: subnetID,
		},
	)
	_, err := r.Extract()
