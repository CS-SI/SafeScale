...
func (s *Stack) CreateHost(request resources.HostRequest) (host *resources.Host, userData *userdata.Content, err error) {
    ...
    defer debug.NewTracer(nil, fmt.Sprintf("(%s)", request.ResourceName), true).WithStopwatch().GoingIn().OnExitTrace()()
            ...
            if err != nil {
                ...
                case gc.ErrDefault409:
                    ...
                    return scerr.AbortedError(
                        "", scerr.Errorf(fmt.Sprintf("error getting host '%s': %s", host.ID, ProviderErrorToString(err)), err),
                    )
