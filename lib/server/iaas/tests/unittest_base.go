...
func (tester *ServiceTester) Networks(t *testing.T) {
	...
	err = gw1.Properties.LockForRead(hostproperty.NetworkV1).ThenUse(
...
func (tester *ServiceTester) Volume(t *testing.T) {
	...
	v1, err := tester.Service.CreateVolume(
	...
	v2, err := tester.Service.CreateVolume(
		abstract.VolumeRequest{
			Name:  "test_volume2",
			Size:  35,
			Speed: volumespeed.HDD,
...
func (tester *ServiceTester) VolumeAttachment(t *testing.T) {
	...
	va1ID, err := tester.Service.CreateVolumeAttachment(
	...
	va2ID, err := tester.Service.CreateVolumeAttachment(
		abstract.VolumeAttachmentRequest{
			Name:     "Attachment2",
			HostID:   host.ID,
			VolumeID: v2.ID,
