...
func init() {
	...
	logFileName := utils.AbsPathify(fmt.Sprintf("$HOME/.safescale/providers%s-session.log", session))
	...
	logFileName = utils.AbsPathify(fmt.Sprintf("$HOME/.safescale/safescaled%s-session.log", session))
