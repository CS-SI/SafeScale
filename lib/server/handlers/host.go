...
//go:generate minimock -i github.com/CS-SI/SafeScale/v22/lib/server/handlers.HostHandler -o mocks/mock_host.go
...
func (handler *hostHandler) List(all bool) (_ abstract.HostList, ferr fail.Error) {
	...
	tracer := debug.NewTracer(handler.job.Task(), tracing.ShouldTrace("handlers.host"), "(%v)", all).WithStopwatch().Entering()
	defer tracer.Exiting()
	defer fail.OnExitLogError(&ferr, tracer.TraceMessage())
...
func (handler *hostHandler) Create(req abstract.HostRequest, sizing abstract.HostSizingRequirements) (_ resources.Host, ferr fail.Error) {
	...
	tracer := debug.NewTracer(handler.job.Task(), tracing.ShouldTrace("handlers.host"), "('%s')", req.ResourceName).WithStopwatch().Entering()
	defer tracer.Exiting()
	defer fail.OnExitLogError(&ferr, tracer.TraceMessage())
...
func (handler *hostHandler) BindSecurityGroup(hostRef, sgRef string, enable resources.SecurityGroupActivation) (ferr fail.Error) {
	...
	tracer := debug.NewTracer(handler.job.Task(), tracing.ShouldTrace("handlers.host"), "(%s, %s)", hostRef, sgRef).WithStopwatch().Entering()
	defer tracer.Exiting()
	defer fail.OnExitLogError(&ferr, tracer.TraceMessage())
...
func (handler *hostHandler) ListSecurityGroups(hostRef string) (_ []*propertiesv1.SecurityGroupBond, ferr fail.Error) {
	...
	tracer := debug.NewTracer(handler.job.Task(), tracing.ShouldTrace("handlers.host"), "(%s)", hostRef).WithStopwatch().Entering()
	defer tracer.Exiting()
	defer fail.OnExitLogError(&ferr, tracer.TraceMessage())
