...
type NetworkHandler interface {
	Create(string, string, []string, bool) (resources.Network, fail.Error)
...
func (handler *networkHandler) Create(name string, cidr string, dnsservers []string, keepOnFailure bool) (network resources.Network, xerr fail.Error) {
	if handler == nil {
	...
	rn, xerr := networkfactory.New(handler.job.GetService())
	if xerr != nil {
		return nil, xerr
	}
