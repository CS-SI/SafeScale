...
func sanitize(in string) (string, error) {
	...
	if !path.IsAbs(sanitized) {
		return "", fail.InvalidRequestReport("exposed path must be absolute")
	}
...
func (handler *shareHandler) Create(
	...
	if shareName == "" {
		return nil, fail.InvalidParameterReport("shareName", "cannot be empty")
	}
	if hostName == "" {
		return nil, fail.InvalidParameterReport("hostName", "cannot be empty")
	}
	if path == "" {
...
func (handler *shareHandler) Delete(name string) (err error) {
	...
	if name == "" {
		return fail.InvalidParameterReport("name", "cannot be empty!")
	}
...
func (handler *shareHandler) Mount(shareName, hostRef, path string, withCache bool) (mount *propertiesv1.HostRemoteMount, err error) {
	...
	if shareName == "" {
		return nil, fail.InvalidParameterReport("shareName", "cannot be empty string")
	}
	if hostRef == "" {
		return nil, fail.InvalidParameterReport("hostRef", "cannot be empty string")
	}
	if path == "" {
	...
	if err != nil {
		return nil, fail.InvalidRequestReport("invalid mount path '%s': '%s'", path, err)
	}
	...
	err = target.Inspect(task, func(_ data.Clonable, props *serialize.JSONProperties) error {
		inErr := props.Inspect(task, hostproperty.MountsV1, func(clonable data.Clonable) error {
			...
			if !ok {
				return fail.InconsistentReport("'*propertiesv1.HostsMounts' expected, '%s' provided", reflect.TypeOf(clonable).String())
			}
			if s, ok := targetMountsV1.RemoteMountsByShareID[objs.SafeGetID()]; ok {
				return fail.DuplicateReport("already mounted in '%s:%s'", target.SafeGetName(), targetMountsV1.RemoteMountsByPath[s].Path)
			}
			for _, i := range targetMountsV1.LocalMountsByPath {
				if i.Path == path {
					...
					return fail.DuplicateReport("there is already a volume in path '%s:%s'", target.SafeGetName(), path)
				}
			}
			for _, i := range targetMountsV1.RemoteMountsByPath {
				if strings.Index(path, i.Path) == 0 {
					...
					return fail.DuplicateReport("there is already a share mounted in '%s:%s'", target.SafeGetName(), i.Path)
				}
			}
			return nil
		})
	...
	err = server.Inspect(task, func(_ data.Clonable, props *serialize.JSONProperties) error {
		...
		inErr := props.Inspect(task, hostproperty.NetworkV1, func(clonable data.Clonable) error {
			...
			if ip == "" {
				return fail.NotFoundReport("no IP address found on server '%s' to serve the share to host '%s'", server.SafeGetName(), target.SafeGetName())
			}
			return nil
		})
		if inErr != nil {
	...
	err = server.Alter(task, func(clonable data.Clonable, props *serialize.JSONProperties) error {
		return props.Alter(task, hostproperty.SharesV1, func(clonable data.Clonable) error {
			...
			if !ok {
				return fail.InconsistentReport("'*propertiesv1.HostShare' expected, '%s' provided", reflect.TypeOf(clonable).String())
			}
			_, found := serverSharesV1.ByID[serverSharesV1.ByName[shareName]]
			if !found {
				return fail.NotFoundReport("failed to find metadata about share '%s'", shareName)
			}
...
func (handler *shareHandler) Unmount(shareRef, hostRef string) (err error) {
	...
	if shareRef == "" {
		return fail.InvalidParameterReport("shareRef", "cannot be empty string")
	}
	if hostRef == "" {
	...
	err = server.Inspect(task, func(_ data.Clonable, props *serialize.JSONProperties) error {
		return props.Inspect(task, hostproperty.SharesV1, func(clonable data.Clonable) error {
			...
			if !found {
				...
				if !found {
					return fail.NotFoundReport("failed to find data about share '%s'", shareRef)
				}
				shareID = share.ID
	...
	err = target.Alter(task, func(_ data.Clonable, props *serialize.JSONProperties) error {
		return props.Alter(task, hostproperty.MountsV1, func(clonable data.Clonable) error {
			...
			if !found {
				return fail.NotFoundReport("not mounted on host '%s'", target.SafeGetName())
			}
...
// If share isn't found, return (nil, nil, nil, utils.NotFound)
