...
func (handler *ShareHandler) Create(
	...
	err = server.Properties.LockForRead(hostproperty.MountsV1).ThenUse(
	...
	err = server.Properties.LockForRead(hostproperty.SharesV1).ThenUse(
		func(clonable data.Clonable) error {
	...
	err = server.Properties.LockForWrite(hostproperty.SharesV1).ThenUse(
		func(clonable data.Clonable) error {
			serverSharesV1 := clonable.(*propsv1.HostShares)
	...
	defer func() {
		if err != nil {
			err2 := server.Properties.LockForWrite(hostproperty.SharesV1).ThenUse(
				func(clonable data.Clonable) error {
					serverSharesV1 := clonable.(*propsv1.HostShares)
...
func (handler *ShareHandler) List(ctx context.Context) (props map[string]map[string]*propsv1.HostShare, err error) {
	...
	err = ms.Browse(
		func(hostName string, shareID string) error {
	...
	if len(shares) == 0 && len(warnings) != 0 {
		return shares, fail.Errorf(
			"there have been problems iterating the list of shares", fail.ErrListError(warnings),
		)
	}
...
func (handler *ShareHandler) Mount(
	...
	tracer := debug.NewTracer(nil, fmt.Sprintf("('%s', '%s')", shareName, hostName), true).WithStopwatch().GoingIn()
	defer tracer.OnExitTrace()()
	defer fail.OnExitLogError(tracer.TraceMessage(""), &err)()
	...
	err = target.Properties.LockForRead(hostproperty.MountsV1).ThenUse(
		func(clonable data.Clonable) error {
	...
	export := ""
	err = target.Properties.LockForRead(hostproperty.NetworkV1).ThenUse(
		func(clonable data.Clonable) error {
	...
	err = target.Properties.LockForWrite(hostproperty.MountsV1).ThenUse(
		func(clonable data.Clonable) error {
			targetMountsV1 := clonable.(*propsv1.HostMounts)
	...
	defer func() {
		if err != nil {
			err2 := target.Properties.LockForWrite(hostproperty.MountsV1).ThenUse(
				func(clonable data.Clonable) error {
					targetMountsV1 := clonable.(*propsv1.HostMounts)
...
func (handler *ShareHandler) ForceInspect(
	...
	if ctx == nil {
		return nil, nil, nil, fail.InvalidParameterError("ctx", "cannot be nil")
	}
	// FIXME: validate parameters
...
func (handler *ShareHandler) Inspect(
	...
	for k := range share.ClientsByName {
		...
		err = client.Properties.LockForRead(hostproperty.MountsV1).ThenUse(
			func(clonable data.Clonable) error {
