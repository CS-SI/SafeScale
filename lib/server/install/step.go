	...
	if is.Serial || s.Serialize {
		...
		for _, h := range hosts {
			...
			if !results[h.Name].Successful() {
				if is.Worker.action == action.Check { // Checks can fail and it's ok
					tracer.Trace(
						"%s(%s):step(%s)@%s finished in %s: not present: %s",
		...
		for k, s := range subtasks {
			...
			if err != nil {
				log.Warn(
					tracer.TraceMessage(
						": %s(%s):step(%s)@%s finished after %s, but failed to recover result",
...
func (is *step) taskRunOnHost(t concurrency.Task, params concurrency.TaskParameters) (result concurrency.TaskResult, err error) {
	...
	variables["TemplateLongOperationTimeout"] = strings.Replace(
		temporal.GetHostTimeout().Truncate(time.Minute).String(), "0s", "", -1,
	)
	variables["TemplatePullImagesTimeout"] = strings.Replace(
