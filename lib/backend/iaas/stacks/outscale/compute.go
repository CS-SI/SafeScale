...
func (instance *stack) ListImages(ctx context.Context, _ bool) (_ []*abstract.Image, ferr fail.Error) {
	if valid.IsNil(instance) {
		return nil, fail.InvalidInstanceError()
	}

	tracer := debug.NewTracer(ctx, tracing.ShouldTrace("stacks.compute") || tracing.ShouldTrace("stack.outscale")).WithStopwatch().Entering()
	defer tracer.Exiting()

	resp, xerr := instance.rpcReadImages(ctx, nil)
	if xerr != nil {
		return nil, xerr
	}
...
func (instance *stack) cpuFreq(perf int) float32 {
	freq := float32(2.0)
	if f, ok := instance.CPUPerformanceMap[perf]; ok {
		freq = f
	}
	return freq
}
...
func (instance *stack) parseTemplateID(id string) (*abstract.HostTemplate, fail.Error) {
...
func (instance *stack) ListTemplates(ctx context.Context, _ bool) (_ []*abstract.HostTemplate, ferr fail.Error) {
	...
	templates := make([]*abstract.HostTemplate, len(instance.templates))
	_ = copy(templates, instance.templates)
	return templates, nil
}

func (instance *stack) buildTemplateList() {
	// without GPU
	...
	templateCap := len(cpus) * len(ramPerCore) * len(perfLevels)
	templateCap = templateCap + templateCap*len(gpus)*3
	instance.templates = make([]*abstract.HostTemplate, 0, templateCap)
	for _, cpu := range cpus {
		for _, ramCore := range ramPerCore {
			for _, perf := range perfLevels {
				...
				instance.templates = append(instance.templates, &abstract.HostTemplate{
					DiskSize:  0,
					Name:      name,
					Cores:     cpu,
					RAMSize:   float32(ram),
...
func (instance *stack) InspectImage(ctx context.Context, id string) (_ *abstract.Image, ferr fail.Error) {
	...
	resp, xerr := instance.rpcReadImageByID(ctx, id)
	if xerr != nil {
		return nil, xerr
	}
...
func (instance *stack) InspectTemplate(ctx context.Context, id string) (_ *abstract.HostTemplate, ferr fail.Error) {
	if valid.IsNil(instance) {
		return nil, fail.InvalidInstanceError()
	}
	if id == "" {
	...
	return instance.parseTemplateID(id)
}

func (instance *stack) getOrCreatePassword(request abstract.HostRequest) (string, fail.Error) {
	if request.Password == "" {
...
func (instance *stack) prepareUserData(ctx context.Context, request abstract.HostRequest, ud *userdata.Content) fail.Error {
	...
	if xerr := ud.Prepare(*instance.configurationOptions, request, cidr, "", timings); xerr != nil {
...
func (instance *stack) createNIC(ctx context.Context, request *abstract.HostRequest, subnet *abstract.Subnet) (osc.Nic, fail.Error) {
	...
	resp, xerr := instance.rpcCreateNic(ctx, name, subnet.ID, description, nil)
	if xerr != nil {
		return osc.Nic{}, xerr
	}
...
func (instance *stack) createNICs(ctx context.Context, request *abstract.HostRequest) (nics []osc.Nic, ferr fail.Error) {
	nics = []osc.Nic{}

	// first network is the default network
	var xerr fail.Error
	nics, xerr = instance.tryCreateNICS(ctx, request, nics)
	if xerr != nil { // if error delete created NICS
		for _, v := range nics {
			xerr := instance.rpcDeleteNic(ctx, v.NicId)
			if xerr != nil {
...
func (instance *stack) tryCreateNICS(ctx context.Context, request *abstract.HostRequest, nics []osc.Nic) ([]osc.Nic, fail.Error) {
	for _, n := range request.Subnets[1:] {
		nic, xerr := instance.createNIC(ctx, request, n)
		if xerr != nil {
			return nics, xerr
		}
...
func (instance *stack) deleteNICs(ctx context.Context, nics []osc.Nic) fail.Error {
	for _, nic := range nics {
		// TODO: parallelize ?
		if xerr := instance.rpcDeleteNic(ctx, nic.NicId); xerr != nil {
			return xerr
		}
	}
	return nil
...
func (instance *stack) hostState(ctx context.Context, id string) (hoststate.Enum, fail.Error) {
	vm, xerr := instance.rpcReadVMByID(ctx, id)
	if xerr != nil {
		switch xerr.(type) {
...
func (instance *stack) WaitHostReady(ctx context.Context, hostParam iaasapi.HostParameter, timeout time.Duration) (*abstract.HostCore, fail.Error) {
	...
	return instance.WaitHostState(ctx, hostParam, hoststate.Started, timeout)
}
...
func (instance *stack) WaitHostState(ctx context.Context, hostParam iaasapi.HostParameter, state hoststate.Enum, timeout time.Duration) (_ *abstract.HostCore, ferr fail.Error) {
	if valid.IsNil(instance) {
		return nil, fail.InvalidInstanceError()
	}
	...
	xerr = retry.WhileUnsuccessfulWithHardTimeout(
		func() error {
			...
			st, innerXErr := instance.hostState(ctx, ahf.ID)
			if innerXErr != nil {
...
func (instance *stack) addNICs(ctx context.Context, request *abstract.HostRequest, vmID string) ([]osc.Nic, fail.Error) {
	if len(request.Subnets) > 1 {
		nics, xerr := instance.createNICs(ctx, request)
		if xerr != nil {
			return nil, xerr
		}
		for i, nic := range nics {
			xerr = instance.rpcLinkNic(ctx, vmID, nic.NicId, int32(i+1))
			if xerr != nil {
...
func (instance *stack) addGPUs(ctx context.Context, request *abstract.HostRequest, tpl abstract.HostTemplate, vmID string) (ferr fail.Error) {
	...
	for gpu := 0; gpu < tpl.GPUNumber; gpu++ {
		resp, xerr = instance.rpcCreateFlexibleGpu(ctx, tpl.GPUType)
		if xerr != nil {
			createErr = xerr
			break
		}
		...
		xerr = instance.rpcLinkFlexibleGpu(ctx, vmID, resp.FlexibleGpuId)
		if xerr != nil {
			break
		}
	}
	if xerr != nil {
		for _, gpu := range flexibleGpus {
			if rerr := instance.rpcDeleteFlexibleGpu(ctx, gpu.FlexibleGpuId); rerr != nil {
...
func (instance *stack) addPublicIP(ctx context.Context, nic osc.Nic) (_ osc.PublicIp, ferr fail.Error) {
	// Allocate public IP
	resp, xerr := instance.rpcCreatePublicIP(ctx)
	if xerr != nil {
	...
	defer func() {
		ferr = debug.InjectPlannedFail(ferr)
		if ferr != nil {
			if derr := instance.rpcDeletePublicIPByID(context.Background(), resp.PublicIpId); derr != nil {
	...
	// Attach public ip
	if xerr = instance.rpcLinkPublicIP(ctx, resp.PublicIpId, nic.NicId); xerr != nil {
		return osc.PublicIp{}, xerr
	}
...
func (instance *stack) setHostProperties(ctx context.Context, ahf *abstract.HostFull, subnets []*abstract.Subnet, vm osc.Vm, nics []osc.Nic) fail.Error {
	vmType, xerr := instance.InspectTemplate(ctx, vm.VmType)
	if xerr != nil {
		return xerr
	}
...
func (instance *stack) initHostProperties(ctx context.Context, request *abstract.HostRequest, host *abstract.HostFull, udc userdata.Content) fail.Error {
	...
	template, err := instance.InspectTemplate(ctx, request.TemplateID)
	if err != nil {
		return err
	}

	host.PrivateKey = udc.FirstPrivateKey
...
func (instance *stack) addPublicIPs(ctx context.Context, primaryNIC osc.Nic, otherNICs []osc.Nic) (osc.PublicIp, fail.Error) {
	ip, xerr := instance.addPublicIP(ctx, primaryNIC)
	if xerr != nil {
		return osc.PublicIp{}, xerr
	}

	for _, nic := range otherNICs {
		if _, xerr = instance.addPublicIP(ctx, nic); xerr != nil {
			return osc.PublicIp{}, xerr
		}
	}
	return ip, nil
...
func (instance *stack) CreateHost(ctx context.Context, request abstract.HostRequest, extra interface{}) (_ *abstract.HostFull, udc *userdata.Content, ferr fail.Error) {
	...
	if len(request.Subnets) == 0 && !request.PublicIP {
		return nil, nil, abstract.ResourceInvalidRequestError("host creation", "cannot create a host without public IP or without attached subnet")
	}
	...
	password, xerr := instance.getOrCreatePassword(request)
	if xerr != nil {
		return nil, nil, xerr
	}
	...
	subnetID, xerr := instance.getDefaultSubnetID(request)
	if xerr != nil {
		return nil, nil, xerr
	}
	...
	if xerr = instance.prepareUserData(ctx, request, udc); xerr != nil {
		return nil, nil, xerr
	}
	...
	if xerr = instance.ImportKeyPair(ctx, creationKeyPair); xerr != nil {
		return nil, nil, xerr
	}

	defer func() {
		ferr = debug.InjectPlannedFail(ferr)
		if derr := instance.DeleteKeyPair(context.Background(), creationKeyPair.Name); derr != nil {
	...
	if xerr = instance.initHostProperties(ctx, &request, ahf, *udc); xerr != nil {
		return nil, nil, xerr
	}
	...
	vmsRequest := osc.CreateVmsRequest{
		...
		Placement: osc.Placement{
			SubregionName: instance.Options.Compute.Subregion,
			Tenancy:       instance.Options.Compute.DefaultTenancy,
		},
	...
	vmsRequest.BlockDeviceMappings = []osc.BlockDeviceMappingVmCreation{
		{
			Bsu: osc.BsuToCreate{
				...
				VolumeType:         instance.fromAbstractVolumeSpeed(instance.Options.Compute.DefaultVolumeSpeed),
			},
			NoDevice:   "true",
			DeviceName: "/dev/sda1",
		},
	...
	xerr = retry.WhileUnsuccessful(
		func() (ferr error) {
			...
			resp, innerXErr := instance.rpcCreateVMs(ctx, vmsRequest)
			if innerXErr != nil {
			...
			defer func() {
				...
				if ferr != nil {
					...
					if derr := instance.DeleteHost(context.Background(), vm.VmId); derr != nil {
			...
			_, innerXErr = instance.WaitHostState(ctx, vm.VmId, hoststate.Started, timings.HostOperationTimeout())
			return innerXErr
		},
		timings.NormalDelay(),
	...
	nics, xerr := instance.rpcReadNics(ctx, "", vm.VmId)
	if xerr != nil {
		return nil, nil, xerr
	}
	if len(nics) == 0 {
	...
	nics, xerr = instance.addNICs(ctx, &request, vm.VmId)
	if xerr != nil {
		return nil, nil, xerr
	}
	if request.PublicIP {
		ip, xerr := instance.addPublicIPs(ctx, defaultNic, nics)
		if xerr != nil {
			return nil, nil, xerr
		}
	...
	if xerr = instance.addGPUs(ctx, &request, *template, vm.VmId); xerr != nil {
		return nil, nil, xerr
	}
	...
	into := map[string]string{
		...
		"ManagedBy":        "safescale",
		"DeclaredInBucket": instance.configurationOptions.MetadataBucketName,
	...
	_, xerr = instance.rpcCreateTags(ctx, vm.VmId, into)
	if xerr != nil {
		return nil, nil, xerr
	}

	_, xerr = instance.WaitHostState(ctx, vm.VmId, hoststate.Started, timings.HostOperationTimeout())
	if xerr != nil {
		return nil, nil, xerr
	}
	...
	xerr = instance.setHostProperties(ctx, ahf, request.Subnets, vm, nics)
	return ahf, udc, xerr
}

func (instance *stack) getDefaultSubnetID(request abstract.HostRequest) (string, fail.Error) {
	if len(request.Subnets) == 0 {
		return "", nil
	}
...
func (instance *stack) deleteHost(ctx context.Context, id string) fail.Error {
	timings, xerr := instance.Timings()
	if xerr != nil {
		return xerr
	}

	if xerr := instance.rpcDeleteVms(ctx, []string{id}); xerr != nil {
		return xerr
	}
	_, xerr = instance.WaitHostState(ctx, id, hoststate.Terminated, timings.HostCreationTimeout())
	return xerr
}
...
func (instance *stack) DeleteHost(ctx context.Context, hostParam iaasapi.HostParameter) (ferr fail.Error) {
	if valid.IsNil(instance) {
	...
	tracer := debug.NewTracer(ctx, tracing.ShouldTrace("stacks.compute") || tracing.ShouldTrace("stack.outscale"), "(%s)", hostLabel).WithStopwatch().Entering()
	defer tracer.Exiting()

	publicIPs, xerr := instance.rpcReadPublicIPsOfVM(ctx, ahf.ID)
	if xerr != nil {
		return fail.Wrap(xerr, "failed to read public IPs of Host with ID %s", ahf.ID)
	}

	// list attached volumes
	volumes, xerr := instance.ListVolumeAttachments(ctx, ahf.ID)
	if xerr != nil {
		volumes = []*abstract.VolumeAttachment{}
	}

	// delete host
	xerr = instance.deleteHost(ctx, ahf.ID)
	if xerr != nil {
		return xerr
	}
	...
	for _, ip := range publicIPs {
		if xerr = instance.rpcDeletePublicIPByID(ctx, ip.PublicIpId); xerr != nil { // continue to delete even if error
			lastErr = xerr
	...
	for _, v := range volumes {
		tags, xerr := instance.rpcReadTagsOfResource(ctx, v.VolumeID)
		if xerr != nil {
			continue
		}
		if _, ok := tags["DeleteWithVM"]; ok {
			if xerr = instance.DeleteVolume(ctx, v.VolumeID); xerr != nil { // continue to delete even if error
...
func (instance *stack) InspectHost(ctx context.Context, hostParam iaasapi.HostParameter) (ahf *abstract.HostFull, ferr fail.Error) {
	if valid.IsNil(instance) {
		return nil, fail.InvalidInstanceError()
	}
	...
	if ahf.ID != "" {
		...
		vm, xerr = instance.rpcReadVMByName(ctx, ahf.Name)
		if xerr != nil {
			return nil, xerr
		}
	}

	return ahf, instance.complementHost(ctx, ahf, vm)
}

func (instance *stack) complementHost(ctx context.Context, ahf *abstract.HostFull, vm osc.Vm) fail.Error {
	ahf.ID = vm.VmId

	tags, xerr := instance.rpcReadTagsOfResource(ctx, vm.VmId)
	if xerr != nil {
		return xerr
	}
	...
	subnets, nics, xerr := instance.listSubnetsByHost(ctx, vm.VmId)
	if xerr != nil {
		return xerr
	}
	xerr = instance.setHostProperties(ctx, ahf, subnets, vm, nics)
	return xerr
}

// GetHostState returns the current state of the host identified by id
func (instance *stack) GetHostState(ctx context.Context, hostParam iaasapi.HostParameter) (_ hoststate.Enum, ferr fail.Error) {
	if valid.IsNil(instance) {
	...
	return instance.hostState(ctx, ahf.ID)
}

// ListHosts lists all hosts
func (instance *stack) ListHosts(ctx context.Context, details bool) (_ abstract.HostList, ferr fail.Error) {
	...
	resp, xerr := instance.rpcReadVMs(ctx, nil)
	if xerr != nil {
...
func (instance *stack) StopHost(ctx context.Context, host iaasapi.HostParameter, gracefully bool) (ferr fail.Error) {
	if valid.IsNil(instance) {
		return fail.InvalidInstanceError()
	}
	...
	return instance.rpcStopVMs(ctx, []string{ahf.ID})
}

// StartHost starts the host identified by id
func (instance *stack) StartHost(ctx context.Context, hostParam iaasapi.HostParameter) (ferr fail.Error) {
	if valid.IsNil(instance) {
		return fail.InvalidInstanceError()
	}
	...
	return instance.rpcStartVMs(ctx, []string{ahf.ID})
}

// RebootHost Reboot host
func (instance *stack) RebootHost(ctx context.Context, hostParam iaasapi.HostParameter) (ferr fail.Error) {
	if valid.IsNil(instance) {
		return fail.InvalidInstanceError()
	}
	ahf, hostRef, xerr := iaasapi.ValidateHostParameter(hostParam)
	...
	return instance.rpcRebootVMs(ctx, []string{ahf.ID})
}

func (instance *stack) perfFromFreq(freq float32) int {
	var perfList sort.IntSlice
	for k := range instance.CPUPerformanceMap {
		perfList = append(perfList, k)
	}
	sort.Sort(sort.Reverse(perfList))
	for _, perf := range perfList {
		if freq < instance.CPUPerformanceMap[perf] {
			return perf
		}
	}
	return 1
...
func (instance *stack) ResizeHost(ctx context.Context, hostParam iaasapi.HostParameter, sizing abstract.HostSizingRequirements) (ahf *abstract.HostFull, ferr fail.Error) {
	if valid.IsNil(instance) {
		return nil, fail.InvalidInstanceError()
	}
	...
	tracer := debug.NewTracer(ctx, true /*tracing.ShouldTrace("stacks.compute") || tracing.ShouldTrace("stack.outscale")*/, "(%s, %v)", hostRef, sizing).WithStopwatch().Entering()
	defer tracer.Exiting()

	perf := instance.perfFromFreq(sizing.MinCPUFreq)
	t := gpuTemplateName(0, sizing.MaxCores, int(sizing.MaxRAMSize), perf, 0, "")

	if xerr := instance.rpcUpdateVMType(ctx, ahf.ID, t); xerr != nil {
		return nil, xerr
	}

	return instance.InspectHost(ctx, ahf.ID)
}
