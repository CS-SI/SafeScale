...
func newNetworkResource(name string) Resource {
	out := &networkResource{NewResourceCore(name)}
	out.ResourceCore.snippet = "snippets/resource_network.tf.template"
	return out
}
...
func (s stack) CreateNetwork(ctx context.Context, req abstract.NetworkRequest) (newNet *abstract.Network, ferr fail.Error) {
	...
	tfExec, xerr := NewSummoner(s.tfWorkdir, global.Settings.Backend.Terraform.ExecPath)
	if xerr != nil {
		return nil, xerr
	}

	xerr = tfExec.CreateMain(s, netRsc.ToMap())
	if xerr != nil {
		return nil, xerr
	}
