...
func (instance stack) DefaultNetwork(context.Context) (*abstract.Network, fail.Error) {
	if valid.IsNil(instance) {
...
func (instance stack) CreateNetwork(ctx context.Context, req abstract.NetworkRequest) (*abstract.Network, fail.Error) {
	if valid.IsNil(instance) {
		return nil, fail.InvalidInstanceError()
	}
	...
	url := instance.NetworkClient.Endpoint + "v1/" + instance.authOpts.ProjectID + "/vpcs" // FIXME: Hardcoded endpoint
...
func (instance stack) ListRouters(ctx context.Context) ([]Router, fail.Error) {
	...
	xerr := stacks.RetryableRemoteCall(ctx,
		func() error {
			return routers.List(instance.NetworkClient, routers.ListOpts{}).EachPage(
...
func (instance stack) findOpenStackNetworkBoundToVPC(ctx context.Context, vpcName string) (*networks.Network, fail.Error) {
	var router *Router
	found := false
	routerList, xerr := instance.ListRouters(ctx)
	if xerr != nil {
	...
	commRetryErr := stacks.RetryableRemoteCall(ctx,
		func() (innerErr error) {
			network, innerErr = networks.Get(instance.NetworkClient, router.NetworkID).Extract()
			return innerErr
		},
		normalizeError,
	)
...
func (instance stack) InspectNetwork(ctx context.Context, id string) (*abstract.Network, fail.Error) {
	...
	r := vpcGetResult{}
	url := instance.NetworkClient.Endpoint + "v1/" + instance.authOpts.ProjectID + "/vpcs/" + id // FIXME: Hardcoded endpoint
	opts := gophercloud.RequestOpts{
		JSONResponse: &r.Body,
		OkCodes:      []int{200, 201},
	}
	...
	commRetryErr := stacks.RetryableRemoteCall(ctx,
		func() (innerErr error) {
			var hr *http.Response
			hr, innerErr = instance.Driver.Request("GET", url, &opts) // nolint
			defer closer(hr)
			if innerErr != nil {
				return innerErr
			}
...
func (instance stack) InspectNetworkByName(ctx context.Context, name string) (_ *abstract.Network, ferr fail.Error) {
	...
	nets, xerr := instance.ListNetworks(ctx)
	if xerr != nil {
		return nil, xerr
	}
...
func (instance stack) DeleteNetwork(ctx context.Context, networkParam iaasapi.NetworkIdentifier) fail.Error {
	...
	if an.ID == "" {
		return fail.InvalidParameterError("networkParam", "invalid empty string in field 'ID'")
	}

	r := vpcCommonResult{}
	url := instance.NetworkClient.Endpoint + "v1/" + instance.authOpts.ProjectID + "/vpcs/" + an.ID // FIXME: Hardcoded endpoint
	opts := gophercloud.RequestOpts{
		JSONResponse: &r.Body,
	...
	return stacks.RetryableRemoteCall(ctx,
		func() (innerErr error) {
			var r *http.Response
			r, innerErr = instance.Driver.Request("DELETE", url, &opts) // nolint
			defer closer(r)
			return innerErr
		},
		normalizeError,
...
func (instance stack) CreateSubnet(ctx context.Context, req abstract.SubnetRequest) (subnet *abstract.Subnet, ferr fail.Error) {
	if valid.IsNil(instance) {
		return nil, fail.InvalidInstanceError()
	}
	...
	an, xerr := instance.InspectNetwork(ctx, req.NetworkID)
	if xerr != nil {
		switch xerr.(type) {
		case *fail.ErrNotFound:
			an, xerr = instance.InspectNetworkByName(ctx, req.NetworkID)
			if xerr != nil {
				return nil, xerr
			}
		default:
	...
	// Creates the subnet
	resp, xerr := instance.createSubnet(ctx, req)
	if xerr != nil {
...
// InspectSubnetByName ...
func (instance stack) InspectSubnetByName(ctx context.Context, networkID, name string) (*abstract.Subnet, fail.Error) {
	if valid.IsNil(instance) {
		return nil, fail.InvalidInstanceError()
	}
	if name = strings.TrimSpace(name); name == "" {
	...
	xerr := stacks.RetryableRemoteCall(ctx,
		func() error {
			var hr *http.Response
			hr, r.Err = instance.NetworkClient.Get(instance.NetworkClient.ServiceURL("subnets?name="+name), &r.Body, &gophercloud.RequestOpts{ // nolint
				OkCodes: []int{200, 203},
			})
			defer closer(hr)
			return r.Err
	...
	if found && len(subnetworks) > 0 {
		...
		return instance.inspectOpenstackSubnet(ctx, id)
	}
	return nil, abstract.ResourceNotFoundError("subnet", name)
}

// InspectSubnet returns the subnet identified by id
func (instance stack) InspectSubnet(ctx context.Context, id string) (*abstract.Subnet, fail.Error) {
	if valid.IsNil(instance) {
		return nil, fail.InvalidInstanceError()
	}
	if id == "" {
	...
	r := subnetGetResult{}
	url := instance.NetworkClient.Endpoint + "v1/" + instance.authOpts.ProjectID + "/subnets/" + id // FIXME: Hardcoded endpoint
	opts := gophercloud.RequestOpts{
		JSONResponse: &r.Body,
		OkCodes:      []int{200, 201},
	}
...
func (instance stack) inspectOpenstackSubnet(ctx context.Context, id string) (*abstract.Subnet, fail.Error) {
	if valid.IsNil(instance) {
		return nil, fail.InvalidInstanceError()
	}
	if id == "" {
	...
	xerr := stacks.RetryableRemoteCall(ctx,
		func() (innerErr error) {
			sn, innerErr = subnets.Get(instance.NetworkClient, id).Extract()
			return innerErr
		},
		NormalizeError,
	)
...
func (instance stack) ListSubnets(ctx context.Context, networkRef string) ([]*abstract.Subnet, fail.Error) {
	var emptySlice []*abstract.Subnet
	if valid.IsNil(instance) {
		return emptySlice, fail.InvalidInstanceError()
	}

	url := instance.NetworkClient.Endpoint + "v1/" + instance.authOpts.ProjectID + "/subnets" // FIXME: Hardcoded endpoint
	if networkRef != "" {
		url += "?vpc_id=" + networkRef
	}

	pager := pagination.NewPager(instance.NetworkClient, url, func(r pagination.PageResult) pagination.Page {
	...
	return retry.Action(
		func() error {
			return stacks.RetryableRemoteCall(ctx,
				func() error {
					var hr *http.Response
					hr, innerErr := instance.Driver.Request("DELETE", url, &opts) // nolint
					defer closer(hr)
					return innerErr
				},
				normalizeError,
...
func (instance stack) createSubnet(ctx context.Context, req abstract.SubnetRequest) (*subnets.Subnet, fail.Error) {
	...
	dnsList := instance.cfgOpts.DNSServers
	if len(dnsList) == 0 {
	...
	url := fmt.Sprintf("%sv1/%s/subnets", instance.NetworkClient.Endpoint, instance.authOpts.ProjectID) // FIXME: Hardcoded endpoint
	opts := gophercloud.RequestOpts{
		JSONBody:     b,
	...
	retryErr := retry.WhileUnsuccessfulWithNotify(
		func() error {
			...
			innerXErr := stacks.RetryableRemoteCall(ctx,
				func() error {
					var hr *http.Response
					hr, innerErr := instance.Driver.Request("GET", fmt.Sprintf("%s/%s", url, subnet.ID), &opts) // nolint
					defer closer(hr)
					return innerErr
				},
				normalizeError,
...
func (instance stack) CreateVIP(ctx context.Context, networkID, subnetID, name string, sgs []string) (*abstract.VirtualIP, fail.Error) {
	if valid.IsNil(instance) {
		return nil, fail.InvalidInstanceError()
	}
	if subnetID == "" {
	...
	as, xerr := instance.InspectSubnet(ctx, subnetID)
	if xerr != nil {
		return nil, xerr
	}

	openstackAS, xerr := instance.InspectSubnetByName(ctx, networkID, as.Name)
	if xerr != nil {
		return nil, xerr
	}
