...
func (s Remediator) DeleteSubnet(ctx context.Context, param iaasapi.SubnetIdentifier) (ferr fail.Error) {
	defer fail.OnPanic(&ferr)

	xerr := s.Stack.DeleteSubnet(ctx, param)
	if xerr != nil {
		xerr.WithContext(ctx)
	}
	return xerr
...
func (s Remediator) CreateHost(ctx context.Context, request abstract.HostRequest, extra interface{}) (_ *abstract.HostFull, _ *userdata.Content, ferr fail.Error) {
	...
	host, content, xerr := s.Stack.CreateHost(ctx, request, extra)
	if xerr != nil {
		xerr.WithContext(ctx)
	}
