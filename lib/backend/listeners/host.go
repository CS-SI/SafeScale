...
func (s *HostListener) Create(inctx context.Context, in *protocol.HostCreateRequest) (_ *protocol.Host, ferr error) {
	...
	if len(in.GetSubnets()) > 0 {
		for _, v := range in.GetSubnets() {
			...
			xerr = metadata.InspectAbstract[*abstract.Subnet](ctx, subnetTrx, func(as *abstract.Subnet) fail.Error {
...
// Resize a host
func (s *HostListener) Resize(inctx context.Context, in *protocol.HostCreateRequest) (_ *protocol.Host, err error) {
	defer fail.OnExitConvertToGRPCStatus(inctx, &err)
