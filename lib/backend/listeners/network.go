...
func (s *NetworkListener) Create(inctx context.Context, in *protocol.NetworkCreateRequest) (_ *protocol.Network, ferr error) {
	...
	job, xerr := prepareJob(inctx, in, fmt.Sprintf("/network/%s/create", networkName))
	if xerr != nil {
		return nil, xerr
	}
	defer job.Close()
...
func (s *NetworkListener) Inspect(inctx context.Context, in *protocol.Reference) (_ *protocol.Network, err error) {
	...
	job, xerr := prepareJob(inctx, in, fmt.Sprintf("/network/%s/inspect", networkRef))
	if xerr != nil {
		return nil, xerr
	}
	defer job.Close()
...
func (s *NetworkListener) Delete(inctx context.Context, in *protocol.NetworkDeleteRequest) (empty *googleprotobuf.Empty, err error) {
	...
	job, xerr := prepareJob(inctx, in.GetNetwork(), fmt.Sprintf("/network/%s/delete", networkRef))
	if xerr != nil {
		return nil, xerr
	}
	defer job.Close()
