...
func NewKongController(ctx context.Context, subnet *Subnet, addressPrimaryGateway bool) (_ *KongController, ferr fail.Error) {
	if ctx == nil {
	...
	if !present {
		...
		results, xerr := featureInstance.Check(ctx, addressedGateway, data.NewMap[string, any]())
		xerr = debug.InjectPlannedFail(xerr)
		if xerr != nil {
			return nil, fail.Wrap(xerr, "failed to check if feature 'edgeproxy4subnet' is installed on gateway '%s'", addressedGateway.GetName())
		}

		castedAddressedGateway, err := lang.Cast[*Host](addressedGateway)
		if err != nil {
			return nil, fail.Wrap(err)
		}

		addressedGatewayTrx, xerr := newHostTransaction(ctx, castedAddressedGateway)
		if xerr != nil {
			return nil, xerr
		}
...
func (k *KongController) Apply(ctx context.Context, rule map[interface{}]interface{}, values *data.Map[string, any]) (_ string, ferr fail.Error) {
	...
	subnetTrx, xerr := newSubnetTransaction(ctx, k.subnet)
	if xerr != nil {
		return "", xerr
	}
...
func (k *KongController) addSourceControl(ctx context.Context, ruleName, url, resourceType, resourceID string, sourceControl map[string]interface{}, v *data.Map[string, any]) fail.Error {
	...
	if kongdata, ok := result["data"].([]interface{}); ok && len(kongdata) > 0 {
		for _, i := range kongdata {
			...
			if !ok {
				return fail.InvalidParameterError("localresult['data']", "is an invalid map[string]")
			}
