...
func TestNetworkSecurityGroups_Replace(t *testing.T) {
	...
	err = nsg2.Replace(network)
	if err == nil {
		t.Error("NetworkSecurityGroups.Replace(abstract.Network{}) expect an error")
		t.FailNow()
	}
	if !strings.Contains(err.Error(), "p is not a *NetworkSecurityGroups") {
		t.Errorf("Expect error \"p is not a *NetworkSecurityGroups\", has \"%s\"", err.Error())
	}
