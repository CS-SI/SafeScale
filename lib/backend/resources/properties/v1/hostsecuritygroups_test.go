...
func TestHostSecurityGroups_Replace(t *testing.T) {
	...
	err = hsg2.Replace(network)
	if err == nil {
		t.Error("HostSecurityGroups.Replace(abstract.Network{}) expect an error")
		t.FailNow()
	}
	if !strings.Contains(err.Error(), "p is not a *HostSecurityGroups") {
		t.Errorf("Expect error \"p is not a *HostSecurityGroups\", has \"%s\"", err.Error())
	}
}
