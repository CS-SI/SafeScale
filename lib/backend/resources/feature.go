...
const OptionFeatureSettings = "feature_settings"

func WithFeatureSettings(s rscapi.FeatureSettings) options.Option {
	return func(o options.Options) fail.Error {
		return o.Store(OptionFeatureSettings, s)
	}
}
...
func NewFeature(ctx context.Context, name string) (_ *Feature, ferr fail.Error) {
	if ctx == nil {
...
// with its content
func NewEmbeddedFeature(ctx context.Context, name string) (_ *Feature, ferr fail.Error) {
	if ctx == nil {
		return nil, fail.InvalidParameterCannotBeNilError("ctx")
	}
	if name == "" {
...
func (instance *Feature) Applicable(ctx context.Context, tg Targetable) (bool, fail.Error) {
	if valid.IsNil(instance) {
...
//   - nil: everything went well
//   - fail.InvalidRequestError: a required parameter is missing (value not provided in externals and no default value defined)
func (instance *Feature) conditionParameters(ctx context.Context, externals data.Map[string, any], target Targetable) fail.Error {
...
func (instance *Feature) determineInstallerForTarget(ctx context.Context, target Targetable, action string) (_ Installer, ferr fail.Error) {
...
func (instance *Feature) Add(ctx context.Context, target Targetable, v data.Map[string, any], opts ...options.Option) (_ rscapi.Results, ferr fail.Error) {
	...
	if !s.AddUnconditionally {
		results, xerr := instance.Check(ctx, target, v, WithFeatureSettings(s))
		xerr = debug.InjectPlannedFail(xerr)
		if xerr != nil {
	...
	results, xerr := installer.Add(ctx, instance, target, myV, WithFeatureSettings(s))
	xerr = debug.InjectPlannedFail(xerr)
	if xerr != nil {
		return nil, xerr
	}
...
func (instance *Feature) Remove(ctx context.Context, target Targetable, v data.Map[string, any], opts ...options.Option) (_ rscapi.Results, ferr fail.Error) {
	defer fail.OnPanic(&ferr)
	...
	results, xerr := installer.Remove(ctx, instance, target, myV, opts...)
	xerr = debug.InjectPlannedFail(xerr)
	if xerr != nil {
		return results, xerr
	}
...
func (instance *Feature) installRequirements(ctx context.Context, t Targetable, v data.Map[string, any], s rscapi.FeatureSettings) fail.Error {
	...
	if len(requirements) > 0 {
		...
		for requirement := range requirements {
			...
			results, xerr := needed.Check(ctx, t, v, WithFeatureSettings(s))
			xerr = debug.InjectPlannedFail(xerr)
			if xerr != nil {
...
// mapSuccessfulHostsInResults creates a map of hosts that are marked as successful in results
func mapSuccessfulHostsInResults(results rscapi.Results) (map[string]struct{}, fail.Error) {
	// Walk through results and register Feature in successful hosts
	successfulHosts := map[string]struct{}{}
	keys, xerr := results.Keys()
	if xerr != nil {
		return nil, xerr
	}

	for _, v := range keys {
		r, xerr := results.PayloadOf(v)
		if xerr != nil {
			return nil, xerr
		}
...
func filterEligibleFeatures(ctx context.Context, target Targetable, filter featureFilter) ([]*Feature, fail.Error) {
	if target == nil {
		return nil, fail.InvalidParameterCannotBeNilError("target")
	}
