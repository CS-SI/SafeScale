...
func executeScript(task concurrency.Task, sshconfig system.SSHConfig, name string, data map[string]interface{}) (string, fail.Error) {
	...
	if !hidesOutput {
		...
		cmd = fmt.Sprintf("bash -c %s;exit ${PIPESTATUS}", filename)
	} else {
		...
		cmd = fmt.Sprintf("export BASH_XTRACEFD=7; bash -c %s 7> /tmp/captured 2>&1;echo ${PIPESTATUS} > /tmp/errc;cat /tmp/captured; rm /tmp/captured;exit `cat /tmp/errc`", filename)
	}
