...
func (g *genericPackager) Check(c api.Component, t api.Target) (bool, api.CheckResults, error) {
	...
	results := api.CheckResults{}

	hostTarget, ok := t.(*HostTarget)
	if !ok {
		clusterTarget, onCluster = t.(*ClusterTarget)
		if !onCluster {
			return false, results, fmt.Errorf("type of target is unknown")
		}
	}
	...
	if specs.IsSet("component.targeting.host") {
		...
		if targeting != "yes" && targeting != "true" {
			return false, results, fmt.Errorf("can't install, component doesn't target a host")
		}
	}
	...
	if onCluster {
		...
		if !specs.IsSet(rootKey + ".uninstall") {
			...
			return false, results, fmt.Errorf(msg, c.DisplayName(), c.FullFileName(), rootKey)
		}
		...
		if masterT != "0" {
			...
			for _, k := range mastersStatus {
				if k != api.ComponentAbsent && k != api.ComponentPresent {
					state = false
				}
			}
		}
	...
	if !specs.IsSet(rootKey + ".package") {
		...
		return false, api.CheckResults{}, fmt.Errorf(msg, c.DisplayName(), c.FullFileName(), rootKey)
	}
...
func (g *genericPackager) Add(c api.Component, t api.Target, v map[string]interface{}) (bool, api.AddResults, error) {
	...
	if onCluster {
		...
		if !specs.IsSet(rootKey + ".uninstall") {
			msg := `syntax error in component '%s' specification file (%s):
					no key '%s.uninstall' found`
			return false, api.AddResults{}, fmt.Errorf(msg, c.DisplayName(), c.FullFileName(), rootKey)
		}
	...
	duration := specs.GetInt(rootKey + ".duration")
	if duration == 0 {
		duration = 5
	}
...
func (g *genericPackager) Remove(c api.Component, t api.Target) (bool, api.RemoveResults, error) {
	...
	if onCluster {
		...
		if !specs.IsSet(rootKey + ".uninstall") {
			msg := `syntax error in component '%s' specification file (%s):
					no key '%s.uninstall' found`
			return false, api.RemoveResults{}, fmt.Errorf(msg, c.DisplayName(), c.FullFileName(), rootKey)
		}
